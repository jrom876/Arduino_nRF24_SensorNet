...
/*
    ...
    in response to specific received messages over the Channel 81 emacs network.
    Right now it just buzzes in response to Channel 81 PIR events, but it is easy enough 
    to respond to any message type.
...
//            delay(250);            
//            buzz_routine_2();
            Serial.println("\nPIR detected: " + String(pir));
          }
          if (gate) {
            pir = 0;
            audio_routine();
//            buzz_routine_2();
            Serial.println("\nGate detected: " + String(gate));
          }
          //send_ping_cmd(nada, chan, rad, PA_LOW); 
          break;
...
void serialCommInput() {
  if (Serial.available()) {
    ...
    if (c == 'A' ) {
      ...
      send_ping_cmd(chan_chg, 81, 5, PA_MIN);
